...
# ─────────────────────────────────────────
# SIDEBAR
# Sıra: 1) Proje Sahibi (profil kartı)
#        2) Yeni Analiz butonu
#        3) Geçmiş chatler (kaydırılabilir)
# ─────────────────────────────────────────
with st.sidebar:

    # ── 1) PROJE SAHİBİ (en üstte) ──────────────────────────
    st.markdown("""
    <div style="padding:22px 16px 18px; border-bottom:1px solid rgba(255,255,255,0.15);">
    ...
    # ── 2) YENİ ANALİZ BUTONU (ortada) ──────────────────────
    st.markdown("<div style='padding:14px 12px 10px;'>", unsafe_allow_html=True)
    ...
    # Ayırıcı
    st.markdown(
        "<div style='height:1px; background:rgba(255,255,255,0.15); margin:0 12px 4px;'></div>",
        unsafe_allow_html=True
    )

    # ── 3) GEÇMİŞ CHATLER (alta kaydırılabilir) ─────────────
    db_sb = load_db()
    ...
    if not has_history:
        ...
        <div style="padding:20px 14px; text-align:center;
            ...
            Henüz geçmiş analiz yok.<br>Yukarıdan yeni bir analiz başlatın.
        </div>
        """, unsafe_allow_html=True)
    else:
