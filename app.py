...
# ─────────────────────────────────────────
# URL PARAM'LARDAN AKSİYON OKU (rerun öncesi)
# ─────────────────────────────────────────
...
html, body, .stApp { font-family: 'DM Sans', sans-serif !important; background: #F8F7FF !important; color: #18172B !important; }

/* Streamlit sidebar ve tüm native chrome'u gizle */
...
/* Ana içerik: custom sidebar için sol boşluk */
...
.sb-new-btn {
    ...
    box-shadow: 0 0 0 1px rgba(196,127,255,0.4), 0 4px 20px rgba(160,64,255,0.7), 0 0 40px rgba(160,64,255,0.35);
    transition: all .2s ease;
}
.sb-new-btn:hover {
    background: linear-gradient(135deg, #D490FF 0%, #B050FF 50%, #9A30FF 100%);
    transform: translateY(-1px);
    box-shadow: 0 0 0 1px rgba(212,144,255,0.5), 0 6px 28px rgba(160,64,255,0.85), 0 0 60px rgba(160,64,255,0.5);
}
...
.sb-empty { padding: 20px 14px; text-align: center; color: rgba(255,255,255,0.35); font-size: 0.72rem; line-height: 1.6; }
...
[data-testid="stChatMessage"]:has([data-testid="stChatMessageAvatarUser"]) [data-testid="stMarkdownContainer"] li { color: #fff !important; }
...
/* ── Welcome ── */
.wlc-title { font-size:2rem; font-weight:700; color:#18172B; text-align:center; margin:1.5rem 0 0.3rem; letter-spacing:-0.02em; }
.wlc-sub   { text-align:center; color:#6B6890; font-size:0.86rem; margin-bottom:1.6rem; }
.sug-card  { background:#fff; border:1.5px solid #E4E0FF; border-radius:12px; padding:12px 14px; margin-bottom:0; }
...
.card-btn button { opacity:0!important; position:absolute!important; inset:0!important; width:100%!important; height:1px!important; min-height:1px!important; border:none!important; background:transparent!important; margin:0!important; padding:0!important; }
.chip-row button { background:#fff!important; color:#5B3FD9!important; border:1.5px solid #D4CFFF!important; border-radius:100px!important; padding:5px 14px!important; font-size:0.75rem!important; font-weight:500!important; }
.chip-row button:hover { background:#EDE9FF!important; border-color:#7C5CFC!important; }
.chat-topbar { display:flex; align-items:center; gap:8px; padding:4px 0 14px; border-bottom:1px solid #E4E0FF; margin-bottom:10px; }
...
[data-testid="stBottom"], [data-testid="stBottomBlockContainer"] {
    position:fixed!important; bottom:0!important; left:260px!important; right:0!important;
    background:#F8F7FF!important; border-top:1px solid #E4E0FF!important;
    padding:10px 24px 8px!important; z-index:999!important;
}
[data-testid="stBottomBlockContainer"] .block-container { max-width:820px!important; padding:0!important; margin:0 auto!important; }
[data-testid="stChatInput"] { background:#fff!important; border:1.5px solid #D4CFFF!important; border-radius:12px!important; box-shadow:0 2px 14px rgba(90,75,200,.08)!important; padding:2px 6px 2px 14px!important; }
[data-testid="stChatInput"]:focus-within { border-color:#7C5CFC!important; box-shadow:0 2px 20px rgba(124,92,252,.16)!important; }
[data-testid="stChatInput"] textarea { background:transparent!important; border:none!important; outline:none!important; box-shadow:none!important; font-family:'DM Sans',sans-serif!important; font-size:0.85rem!important; color:#18172B!important; padding:8px 0!important; resize:none!important; }
[data-testid="stChatInput"] textarea::placeholder { color:#AAA7CC!important; }
[data-testid="stChatInput"] button { background:linear-gradient(135deg,#7C5CFC,#5038C8)!important; border:none!important; border-radius:8px!important; width:34px!important; min-width:34px!important; height:34px!important; box-shadow:0 2px 8px rgba(90,75,200,.35)!important; color:#fff!important; }
[data-testid="stChatInput"] button:hover { background:linear-gradient(135deg,#8F70FF,#6348E0)!important; }
...
if not in_chat:
    ...
    if st.session_state.messages:
        ttl = st.session_state.messages[0].get("title", st.session_state.messages[0]["content"][:50] + "…")
    else:
...
# ─────────────────────────────────────────
st.markdown("""
<div style="position:fixed;bottom:54px;left:260px;right:0;text-align:center;
            font-size:0.6rem;color:#9896B8;font-family:'DM Sans',sans-serif;
            pointer-events:none;z-index:998;padding:0 24px;">
